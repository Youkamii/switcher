...
**直接下載** — 從[下載封存](https://github.com/Youkamii/switcher/releases/tag/v1.8.5)下載 `switcher-win-x64-latest.zip`,解壓縮後執行 `switcher.exe`。(Windows 10/11 64 位元)
...
**直接下載** — 從[下載封存](https://github.com/Youkamii/switcher/releases/tag/v1.8.5)下載 `switcher-mac-arm64-latest.zip`,解壓縮後執行 `switcher.app`。僅支援 Apple Silicon — Intel Mac 請改用下方的[從原始碼建置](#從原始碼建置)安裝。
...
- UI 語言可在右鍵點擊選單列的 W 圖示 → 設定 → 語言中切換(한국어·English·日本語·简体中文·繁體中文·हिन्दी)。
- 開機自動啟動預設開啟 — 可在系統匣 設定 → 開機自動啟動 中關閉(也會顯示在 系統設定 → 登入項目 中)。
- 每次啟動都會檢查新版本並自動更新(下次啟動生效) — 可在系統匣 設定 → 自動更新 中關閉。
...
- UI 語言:在系統匣 → 設定 → 語言中可切換 6 種語言(韓文、英文、日文、簡體中文、繁體中文、印地文)。
- 自動更新、開機自動啟動:在系統匣設定中開關。桌面捷徑僅限 Windows。
...
- 螢幕亮度調整:DISPLAY 區域的每台螢幕滑桿調整真實背光(Windows 透過 DDC/CI,macOS 為內建顯示器)。未開啟 DDC/CI 的螢幕與 Mac 的外接螢幕會顯示不支援的提示。
- TFSD(Token Full Self-Driving):作用中帳號的任一用量視窗(5 Hours、Weekly、Fable 等)達 90% 時,自動切換到瓶頸最低的帳號 — 僅所有視窗均有餘量的帳號可作為目標。若已滿視窗將在 30 分鐘內重置則等待而不切換。在系統匣設定中開啟(預設關閉)。
