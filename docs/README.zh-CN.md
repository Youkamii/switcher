...
- UI 语言可在右键点击菜单栏的 W 图标 → 设置 → 语言中切换（한국어·English·日本語·简体中文·繁體中文·हिन्दी）。
- 开机自启动默认开启 — 可在托盘 设置 → 开机自启动 中关闭（也会显示在 系统设置 → 登录项 中）。
- 每次启动都会检查新版本并自动更新（下次启动生效）— 可在托盘 设置 → 自动更新 中关闭。
...
- UI 语言：在托盘 → 设置 → 语言中可切换 6 种语言（韩语·英语·日语·简体中文·繁体中文·印地语）。
- 自动更新·开机自启动：在托盘设置中开关。桌面快捷方式仅限 Windows。
- GitHub 账号切换：在小组件中切换已登录 gh CLI 的账号 — git push/pull（HTTPS）跟随活动账号。无用量显示。
- 黑屏模式：🌙 按钮或托盘菜单把所有屏幕盖上置顶黑幕。移动鼠标时光标周围像烟雾散开一样透出画面；持续用力晃动鼠标 1~2 秒或按 ESC 退出——光会从最后的光标位置扩散开、揭开黑幕。macOS 上无法覆盖全屏应用。
- 隐藏账号信息：🙈 按钮将卡片上的邮箱和 GitHub 账号名模糊处理 — 防止屏幕共享·截图泄露。再按一次恢复。
- 屏幕亮度调节：DISPLAY 区域的每台显示器滑块调节真实背光（Windows 通过 DDC/CI，macOS 为内置显示器）。未开启 DDC/CI 的显示器和 Mac 的外接显示器会显示不支持的提示。
- TFSD（Token Full Self-Driving）：活动账号的任一用量窗口（5 Hours、Weekly、Fable 等）达到 90% 时，自动切换到瓶颈最低的账号 — 仅所有窗口均有余量的账号可作为目标。若已满窗口将在 30 分钟内重置则等待而不切换。在托盘设置中开启（默认关闭）。
