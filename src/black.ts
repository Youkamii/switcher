...
try {
  ...
  // ── 흔들기 해제 감지 ─────────────────────────────────────────────
  // 2초 창 안에서 고속(0.7px/ms 이상) 방향 반전이 8번 이상 쌓이면 해제.
  // 가로·세로 축을 각각 세므로 좌우·상하·원형 어느 쪽으로 휘둘러도 된다.
  // 조정 이력 (사용자 피드백 3회): 가볍게 움직여도 풀림 → 12/0.8로 강화 →
  // 이번엔 너무 늦게 풀림 → 8/0.7로 완화. 일반적인 이동·짧은 슥슥으로는
  // 여전히 안 풀리는 선을 유지한다.
  const SHAKE_WINDOW_MS = 2000;
  const SHAKE_MIN_SPEED = 0.7;
  const SHAKE_FLIPS = 8;
  let lastX = -1;
  let lastY = -1;
  let lastT = 0;
