...
const ko = {
  ...
  tfsdTooltip:
    "TFSD (Token Full Self-Driving) 작동 중 — 이 계정의 한도가 90%에 닿으면 여유가 가장 많은 계정으로 자동 전환됩니다 (클로드는 Fable 창 기준). 직접 전환하면 자율주행이 해제됩니다. 끄기: 트레이 → 설정 → TFSD 자동 전환",
...
const en: Messages = {
  ...
  tfsdTooltip:
    "TFSD (Token Full Self-Driving) engaged — when this account's limit hits 90%, it auto-switches to the account with the most headroom (judged by the Fable window for Claude). Switching manually disengages it. Turn off: tray → Settings → TFSD auto-switch",
...
const ja: Messages = {
  ...
  tfsdTooltip:
    "TFSD (Token Full Self-Driving) 作動中 — このアカウントの上限が90%に達すると、最も余裕のあるアカウントへ自動切り替えします(Claude は Fable 枠基準)。手動で切り替えると自動運転は解除されます。オフ: トレイ → 設定 → TFSD 自動切り替え",
...
const zhCN: Messages = {
  ...
  tfsdTooltip:
    "TFSD（Token Full Self-Driving）运行中 — 此账号额度达到 90% 时自动切换到余量最多的账号（Claude 以 Fable 窗口为准）。手动切换会解除自动驾驶。关闭：托盘 → 设置 → TFSD 自动切换",
...
const zhTW: Messages = {
  ...
  tfsdTooltip:
    "TFSD(Token Full Self-Driving)運作中 — 此帳號額度達 90% 時自動切換到餘量最多的帳號(Claude 以 Fable 視窗為準)。手動切換會解除自動駕駛。關閉:系統匣 → 設定 → TFSD 自動切換",
...
const hi: Messages = {
  ...
  tfsdTooltip:
    "TFSD (Token Full Self-Driving) सक्रिय — इस अकाउंट की लिमिट 90% पर पहुँचते ही सबसे ज़्यादा गुंजाइश वाले अकाउंट पर स्वतः स्विच होगा (Claude के लिए Fable विंडो)। खुद स्विच करने पर सेल्फ-ड्राइविंग बंद हो जाती है। बंद करें: ट्रे → सेटिंग्स → TFSD ऑटो-स्विच",
